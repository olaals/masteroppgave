...
class DoubleScanV1(Dataset):
    def __init__(self, transforms=None):
        ...
        self.nonzero_paths.sort()

        num_samples_from_ds = 600
        self.segs_paths=         self.segs_paths[:num_samples_from_ds]
        self.imgs_paths_proj=   self.imgs_paths_proj[:num_samples_from_ds]
        self.imgs_paths_l=      self.imgs_paths_l[:num_samples_from_ds]
        self.nonzero_paths = self.nonzero_paths[:num_samples_from_ds]
    ...
    def __getitem__(self, idx):
        ...
        #nonzero = np.array(Image.open(self.nonzero_paths[idx]).convert("RGB"))
        #nonzero = np.max(nonzero, axis=2)
