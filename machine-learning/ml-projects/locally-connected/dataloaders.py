...
def rotate_images(train, gt):
    (h,w) = train.shape[:2]
    (cX, cY) = (w//2, h//2)
    rotation = random.randint(-20, 20)
    M = cv2.getRotationMatrix2D((cX, cY), rotation, 1.0)
    rotated_train = cv2.warpAffine(train, M, (w,h))
    rotated_gt = cv2.warpAffine(gt, M, (w,h))
    return rotated_train, rotated_gt

class CustomDataset(Dataset):
    def __init__(self, train_dir, gt_dir, 
            train_transform=transforms.Compose([
                transforms.ToTensor(),
                transforms.ColorJitter(0.4, 0.4, 0.4, 0.03)
    ...
    def __getitem__(self, index):
        train_image = cv2.imread(self.train_paths[index])
        train_image = cv2.cvtColor(train_image, cv2.COLOR_BGR2RGB)
        gt_image = cv2.imread(self.gt_paths[index], 0)
        train_image, gt_image = rotate_images(train_image, gt_image)
...
if __name__ == '__main__':
    ...
    dataset = CustomDataset(train_dir_path, gt_dir_path )
