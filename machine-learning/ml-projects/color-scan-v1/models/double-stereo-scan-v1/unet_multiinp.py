...
class Unet2D(nn.Module):
    def __init__(self, in_channels, out_channels, channel_ratio=1):
        ...
        self.conv1_llf = self.double_conv(in_channels, ch[0], 7, 3)
        self.conv2_llf = self.double_conv(ch[0], ch[1], 3, 1)
        self.conv3_llf = self.double_conv(ch[1], ch[2], 3, 1)
        self.conv4_llf = self.double_conv(ch[2], ch[3], 3, 1)
    ...
    def __call__(self, x):
        out1_llf = self.conv1_llf(x)
        in2_llf = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1)(out1_llf)
        out2_llf = self.conv2_llf(in2_llf)
        in3_llf = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1)(out2_llf)
        out3_llf = self.conv3_llf(in3_llf)
        in4_llf = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1)(out3_llf)
        out4_llf = self.conv4_llf(in4_llf)
        # downsampling part
        out1 = self.conv1(x)
