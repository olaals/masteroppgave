...
class Unet2D(nn.Module):
    def __init__(self, in_channels, out_channels, channel_ratio=1):
        ...
        self.conv1 = self.double_conv(in_channels, ch[0], 3, 1)
        ...
        self.conv6 = self.double_conv(ch[3], ch[4], 3, 1)

        self.upconv5 = self.double_conv(ch[3]*2, ch[3], 3, 1)
    ...
    def __call__(self, x):
        # downsampling part
        out1 = self.conv1(x)
        in2 = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)(out1)
        out2 = self.conv2(in2)
        in3 = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)(out2)
        out3 = self.conv3(in3)
        in4 = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)(out3)
        out4 = self.conv4(in4)
        in5 = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)(out4)
        out5 = self.conv5(in5)
        in6 = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)(out5)
        out6 = self.conv5(in6)

        up5_in = self.conv_transp54(out6)
        cat5 = torch.cat([up5_in,out5], 1)
        up5_out = self.upconv4(cat5)

        up4_in = self.conv_transp54(up5_out)
