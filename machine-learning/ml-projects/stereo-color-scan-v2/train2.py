...
def check_accuracy(valid_dl, model, loss_fn, acc_fn, classes, tb_writer, seen_train_ex, other_logdir, hparam_log):
    model.eval()
    running_loss = 0.0
    running_acc = 0.0
    running_dice = 0.0
    running_class_dice = 0.0
    save_batch = True
    with torch.no_grad():
    ...
    hparam_log["dice val"].append(average_dice_sc.item())
    hparam_log["acc val"].append(acc.item())
    #average_dice_class_sc = running_class_dice / len(valid_dl.dataset)
    hparam_log["seen train ex val"].append(seen_train_ex)
    hparam_log["loss val"].append(average_loss.item())
    average_dice_class_sc = average_dice_sc
...
def train(model, classes, train_dl, valid_dl, loss_fn, optimizer, scheduler, acc_fn, epochs, tb_writer, hparam_log, other_logdir):
    ...
    hparam_log["seen train ex train"] = []
    hparam_log["seen train ex val"] = []
    hparam_log["acc train"] = []
    hparam_log["acc val"] = []
    hparam_log["dice train"] = []
    hparam_log["dice val"] = []
    hparam_log["loss train"] = []
    hparam_log["loss val"] = []
    ...
    for epoch in range(epochs):
        ...
        for X_batch, Y_batch in train_dl:
            ...
            if step % 50 == 0:
                dice_score_train, _ = mean_dice_score(outputs, Y_batch, classes)
                with torch.no_grad():
                    hparam_log["seen train ex train"].append(seen_train_ex)
                    hparam_log["acc train"].append(acc.item())
                    hparam_log["loss train"].append(loss.item())
                    hparam_log["dice train"].append(dice_score_train.item())
            if step % 100 == 0:
                print("step")
        ...
        avg_dice, avg_dice_cl = check_accuracy(valid_dl, model, loss_fn, acc_fn, classes, tb_writer, seen_train_ex, other_logdir, hparam_log)
...
def init_test(cfg):
    ...
    if continue_training:
        ...
        unet.load_state_dict(torch.load(os.path.join("logdir/other/double-stereo-scan-v1/unet_normal/N30_unmesmerized", "state_dict.pth")))
    ...
    array_save_dir = os.path.join(other_logdir, "arrays")
    os.makedirs(array_save_dir, exist_ok=True)



    seen_train_ex_train = hparam_log["seen train ex train"]
    np.save(os.path.join(array_save_dir, "seen_train_ex_train.npy"), seen_train_ex_train)
    seen_train_ex_val = hparam_log["seen train ex val"]
    np.save(os.path.join(array_save_dir, "seen_train_ex_val.npy"), seen_train_ex_val)
    acc_train = hparam_log["acc train"]
    np.save(os.path.join(array_save_dir, "acc_train.npy"), acc_train)
    acc_val = hparam_log["acc val"]
    np.save(os.path.join(array_save_dir, "acc_val.npy"), acc_val)
    dice_train = hparam_log["dice train"]
    np.save(os.path.join(array_save_dir, "dice_train.npy"), dice_train)
    dice_val = hparam_log["dice val"]
    np.save(os.path.join(array_save_dir, "dice_val.npy"), dice_val)
    loss_train = hparam_log["loss train"]
    np.save(os.path.join(array_save_dir, "loss_train.npy"), loss_train)
    loss_val = hparam_log["loss val"]
    np.save(os.path.join(array_save_dir, "loss_val.npy"), loss_val)

    
    plt.figure()
    plt.plot(seen_train_ex_train, loss_train, label="Train")
    plt.plot(seen_train_ex_val, loss_val, label="Validation")
    plt.savefig(os.path.join(array_save_dir, "loss.png"))
    plt.ylabel("Cross entropy loss")
    plt.xlabel("Training steps")
    plt.figure()
    plt.plot(seen_train_ex_train, dice_train, label="Train")
    plt.plot(seen_train_ex_val, dice_val, label="Validation")
    plt.xlabel("Training steps")
    plt.ylabel("Dice score")
    plt.savefig(os.path.join(array_save_dir, "dice.png"))
    plt.figure()
    plt.plot(seen_train_ex_train, acc_train, label="Train")
    plt.plot(seen_train_ex_val, acc_val, label="Validation")
    plt.ylabel("Pixel accuracy")
    plt.xlabel("Training steps")
    plt.savefig(os.path.join(array_save_dir, "acc.png"))
