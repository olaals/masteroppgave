...
class DoubleScanV1(Dataset):
    def __init__(self, transforms=None):
        ...
        self.img_l_paths = [os.path.join(dataset_dir, scan_dir, "stereo-filt.png") for scan_dir in os.listdir(dataset_dir)]
        self.proj_paths = [os.path.join(dataset_dir, scan_dir, "filt_proj_rgb.png") for scan_dir in os.listdir(dataset_dir)]
        self.segs_paths = [os.path.join(dataset_dir, scan_dir, "gt_proj_mask.png") for scan_dir in os.listdir(dataset_dir)]
        self.img_l_paths.sort()
        self.proj_paths.sort()
        self.segs_paths.sort()

        num_samples_from_ds = 4000
        self.segs_paths=         self.segs_paths[:num_samples_from_ds]
        self.img_l_paths=   self.img_l_paths[:num_samples_from_ds]
        self.proj_paths=   self.proj_paths[:num_samples_from_ds]
        

    def __len__(self):
        return len(self.img_l_paths)

    def __getitem__(self, idx):

        img= np.array(Image.open(self.img_l_paths[idx]).convert("RGB"))
        #img2= np.array(Image.open(self.proj_paths[idx]).convert("RGB"))
