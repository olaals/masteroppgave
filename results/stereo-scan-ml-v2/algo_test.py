...
def test_dataset(dataset_path, test_path):
    ...
    right_scans_paths = get_named_image_from_dirs(dataset_path, "img_r.png")
    ...
    for idx in range(len(gt_paths)):
        ...
        right_scan_path = right_scans_paths[idx]
        proj_path = proj_scans_paths[idx]
        ...
        right_scan = cv2.cvtColor(cv2.imread(right_scan_path), cv2.COLOR_BGR2RGB)
        left_scan_gray = cv2.imread(left_scan_path, 0)
        left_scan_gray = np.where(left_scan_gray>15, left_scan_gray, 0)
        proj_gray = cv2.imread(proj_path,0)
        proj_gray = np.where(proj_gray>15, proj_gray, 0)

        combined_proj = np.dstack((proj_gray, left_scan_gray, np.zeros_like(proj_gray)))
        combined_proj = combined_proj.astype(np.uint16)
        combined_proj *= 2
        combined_proj = combined_proj.astype(np.uint8)
        #plt.imshow(combined_proj)
        #plt.show()
        ...
        cv2_imwrite(os.path.join(result_scan_dir, "right_scan.png"), right_scan)
        cv2_imwrite(os.path.join(result_scan_dir, "stacked.png"), stacked)
        cv2_imwrite(os.path.join(result_scan_dir, "stacked_input.png"), combined_proj)
