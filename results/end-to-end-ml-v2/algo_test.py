...
def test_dataset(dataset_path, test_path):
    ...
    outlier_frac_list *=100

    dice_scores_np = np.array(dice_score_list)
    avg_dice_score = np.average(dice_scores_np)
    std_dice_score = np.std(dice_scores_np)

    avg_subpix_score = np.mean(subpix_score_list)
    std_subpix  = np.std(subpix_score_list)
    avg_outlier_frac = np.mean(outlier_frac_list)
    std_outlier_frac =np.std(outlier_frac_list)

    outlier_frac_list.sort()
    dice_score_list.sort()
    subpix_score_list.sort()

    print("Avg dice score", avg_dice_score, "std", std_dice_score)
    print("avg subpix score", avg_subpix_score, "std:", std_subpix)
    print("avg outlier frac", format(avg_outlier_frac, "02f"), "%", "std:", std_outlier_frac)
